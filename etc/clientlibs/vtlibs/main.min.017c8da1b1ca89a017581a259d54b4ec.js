$(document).ready(function(){if($(".vtctalink > .inline").length>0){$(".vtctalink > .inline").parent(".vtctalink").addClass("inline")
}});
(function(c,e){var a=2,b="Can only add 2 additional Calls to Action.";
function f(j,g){j=(j&&(j!==" "))?j:"";
var i=j.split(" ").map(function(o,m,n){var l=(g.tagName)?g.tagName:"span";
var k="";
if(g.child){k=f(o,g.child)
}return"<"+l+">"+(k||o)+"</"+l+">"
});
var h=i.map(function(m,k,l){if(g.attributes){for(var n in g.attributes){m=c(m).attr(n,g.attributes[n])
}}return m[0].outerHTML||m
});
return h
}var d=function(h){var m=c(h).attr("srcset");
var l="";
if(m){var n=m.split(",").map(function(q,o,p){return q.trim().split(" ")
});
var k=window.innerWidth;
if(n.length>0){for(var j=0;
j<n.length;
j++){var g=parseInt(n[j][1],10);
if(g>=k){l=n[j][0]
}else{if(k>=1800){l=n[0][0]
}}}}}else{l=c(h).attr("src")
}return l
};
e.ready(function(){if(c(".vt-callToAction").length>0){c(".vt-callToAction").each(function(){var i=c(this).find(".vt-callToAction-supplement");
if(c(this).find(".vt-video").length<=0){c(this).find(".vt-callToAction-supplement-fig-container").show();
var g=d(i.find("img")[0]);
var h="center";
if(c(this).hasClass("image-right")){h="75%"
}else{if(c(this).hasClass("image-left")){h="25%"
}}c(this).find(".vt-callToAction-supplement-fig").css({background:"url("+g+")","background-size":"cover","background-repeat":"no-repeat","background-position":h})
}});
c(".vt-callToAction-supplement-fig").animate({opacity:1},200)
}if(c(".vt-callToAction .vt-callToAction-textHighlight").length>0){c(".vt-callToAction .vt-callToAction-textHighlight").each(function(){var g=c(this).find("h3");
g.hide();
var h=f(g.html().trim(),{tagName:"span",attributes:{"class":"vt-callToAction-message-header-fragment"},child:{tagName:"span"}});
g.html(h.join(" "));
g.show()
})
}});
e.on("dialog-ready",function(){var h=0,n=c("<span class='coral-Form-fielderror coral-Icon coral-Icon--alert coral-Icon--sizeS'data-init='quicktip'data-quicktip-type='error'</span>"),j=c("#validator");
var k=c(".coral-Multifield");
k.each(function(){if(c(this).find("[name*='./secondaryCTA']").length>0){var o=c(this);
c(this).on("click",".js-coral-Multifield-add",function(p){h++;
l(o)
});
c(this).on("click",".js-coral-Multifield-remove",function(p){h--;
l(o)
})
}});
c.validator.register({selector:j,validate:m,show:i,clear:g});
function l(q){var p=q;
j.checkValidity();
j.updateErrorUI();
var o=p.find(".js-coral-Multifield-add");
if(h>=a){o.attr("disabled","disabled")
}else{o.removeAttr("disabled")
}}function i(p,q){var o=$secondaryCTA.closest("form").hasClass("coral-form--vertical")?"right":"top";
n.clone().attr("data-quicktip-arrow",o).attr("data-quicktip-content",b).insertAfter($secondaryCTA)
}function g(){$secondaryCTA.nextAll(".coral-Form-fieldError").tooltip("hide").remove()
}function m(){if(h>a){return b
}}})
})(jQuery,jQuery(document));
function vt_full_width_sizer(){var a=$("body").width();
$(".vt-full-width").each(function(b,c){var d=$(this).parent().offset();
$(this).css("width",(a));
$(this).css("margin-left",("-"+d.left+"px"));
$(this).css("left","0")
})
}$(document).ready(vt_full_width_sizer);
$(window).resize(vt_full_width_sizer);
$(document).ready(function(){if($(".vt-gallery-type-lbgallery").length>0||$(".vt-gallery-type-featured-lbgallery").length>0){if(!$("#blueimp-gallery").length>0){var a='<div id="blueimp-gallery" class="blueimp-gallery blueimp-gallery-controls" data-use-bootstrap-modal="false" >                    <div class="slides"></div>                    <h3 class="title"></h3>                    <a class="prev"><span class="glyphicon glyphicon-triangle-left"></span></a>                    <a class="next"><span class="glyphicon glyphicon-triangle-right"></span></a>                    <a class="close">×</a>                    <a class="play-pause"></a>                    <ol class="indicator"></ol>                    <div class="modal fade">                        <div class="modal-dialog">                            <div class="modal-content">                                <div class="modal-header">                                    <button type="button" class="close" aria-hidden="true">&times;</button>                                    <h4 class="modal-title"></h4>                                </div>                                <div class="modal-body next"></div>                                <div class="modal-footer">                                    <button type="button" class="btn btn-default pull-left prev">                                        <i class="glyphicon glyphicon-chevron-left"></i>                                        Previous                                    </button>                                    <button type="button" class="btn btn-primary next">                                        Next                                        <i class="glyphicon glyphicon-chevron-right"></i>                                    </button>                                </div>                            </div>                        </div>                    </div>                </div>';
$("body").append(a)
}if(!(jQuery.isFunction(jQuery.fn.Gallery))){$("head").prepend('<script type="text/javascript" src="//www.assets.cms.vt.edu/js/blueimp-gallery/3.0/jquery.blueimp-gallery.min.js"><\/script>');
